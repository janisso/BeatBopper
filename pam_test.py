...
def play_midi(save_path, beats, midi_vel, stop_all,midi_device_nr):
    f = open(save_path + '/play_midi.csv', 'w+')                # open file to save log values
    f.write('time,beats,midi_note,midi_vel\n')                  # write first line with corresponding titles
    mid = lib.mido.MidiFile('/Users/mb/Desktop/Janis.so/06_qmul/BeatBopper/users/analysis/DRIFT/midi_drift.mid')                          # save parsed MIDI file using mido library
...
def play(save_path,midi_device, tempo_method, countoff):
    ...
    ######DECLARE PROCESSES
    p_play_midi = lib.multiprocessing.Process(target=play_midi,args=(save_path,beats,midi_vel,stop_all,midi_device_nr))  # process to play MIDI
